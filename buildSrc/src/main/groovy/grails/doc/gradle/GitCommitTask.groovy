...
class GitCommitTask extends DefaultTask {
    ...
    @Input String getMessage() { message?.toString() }
    @Input boolean commitAll
    @Input @Optional List committer
    @Input @Optional List author

    @TaskAction
    def clone() {
        def git = Git.open(repository)
        def status = git.status().call()
        if (!status.added && !status.changed && !status.removed) {
            return
        }

        def cmd = Git.open(repository).commit()
        cmd.message = message
        cmd.with {
            all = commitAll
            if (committer) setCommitter *committer
