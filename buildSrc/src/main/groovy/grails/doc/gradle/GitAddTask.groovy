...
/**
 * Gradle task for doing equivalent of 'git add'.
 */
class GitAddTask extends DefaultTask {
    ...
    @Input boolean ignoreUntracked
    @Input List getPaths() { paths*.toString() }

    @TaskAction
    def clone() {
        def cmd = Git.open(repository).add()
        for (path in paths) {
            cmd.addFilepattern path
        }
        cmd.update = ignoreUntracked
        cmd.call()
    }
}
